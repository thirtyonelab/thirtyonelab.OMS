...
export default function PaymentVoucherModal({ isOpen, onClose, entry, settings }) {
  ...
  const voucherNo = `PV-${entry.date}-${entry.id.substring(0,4).toUpperCase()}`;
  ...
  return (
    <div className="modal-overlay print-modal-overlay" onClick={onClose}>
      <div className="modal-content A4-modal-container" onClick={e => e.stopPropagation()} style={{ maxWidth: '800px' }}>
        ...
        {/* Top Toolbar */}
        <div className="modal-header print-controls no-print" style={{ padding: '0.75rem 1.25rem', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', boxSizing: 'border-box' }}>
          ...
          >
            <div className="A4-sheet" style={{
              ...
              <div style={{ marginTop: '1.5rem', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '0.3rem' }}>
                <h2 style={{ fontSize: '1.2rem', fontWeight: 800, margin: 0 }}>PAYMENT VOUCHER</h2>
                ...
                <div style={{ fontSize: '0.85rem', marginBottom: '1.5rem', borderBottom: '1px solid #111', paddingBottom: '0.5rem' }}>
                  <strong>PAY TO:</strong> <span style={{ marginLeft: '0.5rem', fontSize: '0.9rem' }}>{entry.payee || 'Cash / Supplier'}</span>
                </div>

                {/* Details Title */}
                <h3 style={{ fontSize: '0.8rem', fontWeight: 'bold', margin: '0.5rem 0' }}>PAYMENT DETAILS:</h3>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '0.5rem' }}>
                  <thead>
                    <tr style={{ borderTop: '1px solid #111', borderBottom: '1px solid #111', backgroundColor: '#f8fafc', fontSize: '0.75rem', fontWeight: 'bold' }}>
                      ...
                      <th style={{ padding: '0.5rem', textAlign: 'left', borderRight: '1px solid #ddd' }}>CATEGORY</th>
                  ...
                  <tbody>
                    <tr style={{ borderBottom: '1px solid #111', fontSize: '0.8rem' }}>
                      <td style={{ padding: '0.5rem', textAlign: 'center', borderRight: '1px solid #ddd' }}>1</td>
                      <td style={{ padding: '0.5rem', borderRight: '1px solid #ddd' }}>{entry.category}</td>
                      <td style={{ padding: '0.5rem', borderRight: '1px solid #ddd' }}>{entry.description}</td>
                      <td style={{ padding: '0.5rem', textAlign: 'right', fontWeight: 'bold' }}>RM {formatRM(entry.amount)}</td>
                    </tr>
                    <tr style={{ fontSize: '0.85rem', fontWeight: 'bold' }}>
                      <td colSpan="3" style={{ padding: '0.75rem 0.5rem', textAlign: 'right' }}>TOTAL AMOUNT:</td>
                      <td style={{ padding: '0.75rem 0.5rem', textAlign: 'right', fontSize: '0.95rem' }}>RM {formatRM(entry.amount)}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              ...
              <div style={{ marginTop: 'auto', paddingTop: '1.5rem', borderTop: '1px solid #e6e2dc' }}>
                <div style={{ fontSize: '0.7rem', color: '#555', marginBottom: '1rem' }}>
                  ...
                  <p style={{ margin: '0.1rem 0 0 0', lineHeight: '1.3' }}>
                    Official voucher for company financial records and operating expenses.
                  </p>
                </div>
